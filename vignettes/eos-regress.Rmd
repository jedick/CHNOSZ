...
As recommended by @ZZL_16, CHNOSZ uses data for `r sio2`(aq) from @AS04, superseding the values from @SHS89 used in earlier versions of the package.
...
On the other hand, the small differences at low temperature stand in contrast to the comparison shown in Figure 3 of Stefánsson (2001), suggesting that the data for `r sio2` from Shock et al. (1989) are not compatible with the most recent updates.

The following example uses the `pseudo-H4SiO4` from Stefánsson (2001) together with values for gibbsite from [Robie et al., 1978](http://pubs.er.usgs.gov/publication/b1452) (as recommended by Zimmer et al., 2016) to make an activity diagram for the K<sub>2</sub>O-Al<sub>2</sub>O<sub>3</sub>-SiO<sub>2</sub>-H<sub>2</sub>O system.
