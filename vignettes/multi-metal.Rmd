...
The next code chunk prepends `@` to the reference keys and uses the chunk option [`results = 'asis'`](https://pkg.yihui.org/rmarkdown-cookbook/results-asis.html) to insert the citations into the R Markdown document in chronological order.
...
# Load SLOP98 data to use legacy parameters for CuCl2- and CuCl3-2
add.OBIGT("SLOP98")
...
In this case, they are equivalent because we did not specify pH in the call to `NaCl()`.
```{r NaCl}
# Ionic strength
NaCl$IS
# Molality of Cl-
...
Here are the `logK` values after the adjustment, compared with the values calculated from the SLOP98 database [@SSH97].
This is followed by a `reset()` call to use load the default database for the rest of this vignette.
