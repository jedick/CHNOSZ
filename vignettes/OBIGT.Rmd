...
This [vignette](index.html), produced on `r Sys.Date()`, lists the citations for thermodynamic data in the OBIGT database in CHNOSZ version `r sessionInfo()$otherPkgs$CHNOSZ$Version`.
...
Each button below corresponds to one of the CSV data files in the `extdata/OBIGT` package directory.
Clicking on a button opens a list of primary citations (`ref1` in `thermo()$OBIGT`) in chronological order; any secondary citations (`ref2`) are listed with bullet points.
Each citation is followed by the number of species, and a note taken from `thermo()$refs`.

Abbreviations: Cp (heat capacity), GHS (standard Gibbs energy, enthalpy, entropy), HKF (Helgeson-Kirkham-Flowers equations), V (volume), T (temperature), P (pressure).
...
Use `add.OBIGT("OldAA")` to load the data.
