...
setfile <- function(csvfile, dat=NULL) {
  ...
  if(csvfile %in% c("SLOP98", "testing")) {
    # Deal with SLOP98 or testing optional data files as a group
    if(csvfile == "SLOP98") group_files <- dir(system.file("extdata/OBIGT", package = "CHNOSZ"), "SLOP98", full.names = TRUE)
    if(csvfile == "testing") group_files <- dir(system.file("extdata/OBIGT/testing", package = "CHNOSZ"), full.names = TRUE)
    group_files <- dir(system.file("extdata/OBIGT", package = "CHNOSZ"), "SLOP98", full.names = TRUE)
    group_list <- lapply(group_files, function(file) {
      dat <- read.csv(file, as.is = TRUE)
      source <- gsub(".csv", "", basename(file))
      cbind(source = source, dat)
    })
    dat <- do.call(rbind, group_list)
    state <- "Optional "
    class <- ifelse(csvfile == "SLOP98", "SLOP98", "Testing")
  } else {
    # Deal with individual data files
...
### Gases <button id="B-gas-inorganic" onclick='ToggleDiv("gas-inorganic")'>Inorganic</button> <button id="B-gas-organic" onclick='ToggleDiv("gas-organic")'>Organic</button> &nbsp;&nbsp;&nbsp;Liquids <button id="B-liquid-inorganic" onclick='ToggleDiv("liquid-inorganic")'>Inorganic</button> <button id="B-liquid-organic" onclick='ToggleDiv("liquid-organic")'>Organic</button>
...
*<strong><a href = "#showall" onclick='OpenAllDivs()'>Click here</a> to show all citations.</strong>*
</div>
...
**NOTE:** Aqueous U species from @SSB97 (with the exception of HUO<sub>4</sub><sup>-</sup>) have been moved to the SLOP98 optional data file due to incompatibility with more recent data [@MBA+25]. Because the latter source includes only uranyl species, the set of data for aqueous uranium species in the default database is currently incomplete.
...
<div id="D-liquid-inorganic" style="display: none">
## <a id="liquid-inorganic" class="anchor"></a> `r setfile("inorganic_liq.csv")`
...
## <a id="optional-SLOP98" class="anchor"></a> `r setfile("SLOP98")`
These species, whose parameters are listed in slop98.dat (or later versions) and were present in earlier versions of CHNOSZ,
have been replaced by or are incompatible with species in the default database.
...
Ga and In [@HB24],
U [@MBA+25],
KHSO<sub>4</sub> and KSO<sub>4</sub><sup>-</sup> [@TAT+25],
Nd<sup>+3</sup> [@FG25].

NOTE: The entire datasets from several studies that are part of or depend on slop98.dat (not restricted to the elements listed above) are kept here.
These datasets include estimated thermodynamic parameters for REE complexes and metal-organic complexes.
The studies are: @SK93, @SK95, @HSS95, @PSK99, @MA07, @AML10, and @AKAE19.
Keeping the datasets in one place rather than splitting them between default and optional files makes the database eaiser to maintain.
