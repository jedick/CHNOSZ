...
3. For the highest-temperature polymorph, if `T` is negative, the opposite (positive) value is the *T* limit of the *C~p~* equation. (*New feature in development version of CHNOSZ*)
...
The purpose of this behavior is to make it harder to obtain potentially unreliable results at temperatures where a mineral (or an organic solid or liquid) is not stable.
If you want `subcrt()` to output the extrapolated Δ*G*° of a phase above its listed temperature limit, then add `exceed.Ttr = TRUE` to the function call.
...
# Remove minerals with no T limit
...
Tmax in the SUPCRT92 database does not in general correspond to the melting or decomposition temperature of a given mineral, and SUPCRT92 outputs Δ*G*° and other standard thermodynamic properties at higher temperatures despite the warning.

This behavior is new in the development version of CHNOSZ (to be 2.0.1).
...
**4. Finally, if `T` is NA, then no upper temerature limit is imposed by `subcrt()`.**
